 ...
 */
const timers = {
    1: { intervalId: null, remainingSeconds: 0, isRunning: false, isFinished: false, originalSeconds: 0, beepIntervalId: null, autoStopTimeoutId: null },
    2: { intervalId: null, remainingSeconds: 0, isRunning: false, isFinished: false, originalSeconds: 0, beepIntervalId: null, autoStopTimeoutId: null },
    3: { intervalId: null, remainingSeconds: 0, isRunning: false, isFinished: false, originalSeconds: 0, beepIntervalId: null, autoStopTimeoutId: null }
};
...
/**
 * アラーム自動停止までの秒数（1分 = 60秒）
 */
const ALARM_AUTO_STOP_SECONDS = 60;
...
function resetTimer(id) {
    ...
    // 自動停止タイマーをクリア
    if (timer.autoStopTimeoutId) {
        clearTimeout(timer.autoStopTimeoutId);
        timer.autoStopTimeoutId = null;
    }
...
function timerFinished(id) {
    ...
    // === 1分（60秒）後に自動でアラートを停止する ===
    timer.autoStopTimeoutId = setTimeout(() => {
        // まだアラート中なら自動停止
        if (timer.isFinished) {
            dismissAlert(id);
        }
    }, ALARM_AUTO_STOP_SECONDS * 1000);
}

/**
 * アラートを停止する（アラート停止ボタン押下時、または自動停止時）
 * @param {number} id - タイマー番号（1, 2, 3）
 */
...
function playAlarmSound(id) {
    ...
    // タイマーごとに異なる音の高さを設定（聞き分けやすくする）
    const frequencies = {
        1: { high: 880, low: 780 },   // Timer 1: コーラル系 → A5/G5
        2: { high: 660, low: 590 },   // Timer 2: ブルー系 → E5/D5
        3: { high: 1050, low: 940 }   // Timer 3: ラベンダー系 → C6/Bb5
    };

    const freq = frequencies[id] || { high: 880, low: 780 };
    ...
    const beepPattern = setInterval(() => {
        ...
        // ビープ音を再生（タイマーごとに異なるパターン）
        playBeep(freq.high, 0.15);
        setTimeout(() => {
            if (timers[id].isFinished) {
                playBeep(freq.high, 0.15);
            }
    ...
    playBeep(freq.high, 0.15);
    setTimeout(() => {
        if (timers[id].isFinished) playBeep(freq.high, 0.15);
    }, 200);
    setTimeout(() => {
        if (timers[id].isFinished) playBeep(freq.low, 0.2);
    }, 400);
}
