...
btnTry.addEventListener('click', () => {
    updateSettingsImmediate(); // 【修正】クリック瞬間の値を強制的に再取得
    showModal();
});

// 【修正】入力値をクリック時に即座に反映させるための関数
function updateSettingsImmediate() {
    currentSettings.title = inputTitle.value;
    currentSettings.content = inputContent.value;
    currentSettings.color = inputColor.value;
    currentSettings.animation = inputAnimation.value;
    // ※画像URLはテキスト入力があれば取得。画像ファイル(FileReader)はchangeイベントで処理済みとする
    if (inputImageUrl.value) {
        currentSettings.imageUrl = inputImageUrl.value;
    }
}
...
function showModal() {
    ...
    // 【修正】強制的なリフロー（スタイルの再計算）を発生させてアニメーションをリセットする
    // これにより、同じアニメーションでも再度クラスを付けたときに動き出す場合がある
...
btnGenerate.addEventListener('click', () => {
    updateSettingsImmediate(); // 生成時も最新の値を取得
...
function generateCode() {
    ...
    // 画像タグの生成 (画像がある場合のみ)
    // 【修正】生成コードにもレスポンシブスタイルを反映
    const imgStyle = "max-width:100%; max-height:40vh; object-fit:contain; display:block; margin:10px auto; border-radius:4px;";
    const imgTag = imageUrl ? `<img src="${imageUrl}" alt="Modal Image" style="${imgStyle}">` : '';
