...
class FastingTimer {
    ...
    playChime() {
        try {
            ...
            gain1.gain.exponentialRampToValueAtTime(0.001, now + 1.5); // 減衰
            ...
            // 2音目「ポーン」 (Dong) - 低い音 (698Hz F5)
            const start2 = now + 0.6;
            const osc2 = ctx.createOscillator();
            const gain2 = ctx.createGain();
            osc2.type = 'sine';
            osc2.frequency.setValueAtTime(698.46, start2);

            gain2.gain.setValueAtTime(0, start2);
            gain2.gain.linearRampToValueAtTime(0.5, start2 + 0.05);
            gain2.gain.exponentialRampToValueAtTime(0.001, start2 + 2.5); // 長い余韻
            ...
            // 本番環境ではエラーが出てもアラートは出さず、静かに失敗させる（UX阻害防止）か、
            // consoleのみに出力するのが一般的だが、要件に従いエラーハンドリングは維持
            console.log('Sound playback failed, likely due to browser interaction policy.');
        }
    }
}
